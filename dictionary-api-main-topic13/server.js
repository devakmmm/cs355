...
//what are envionmental variables? key value pairs that are set outside of the application usually in the operating system
...
function call_dictionary_service(word, res) {
    ...
    const dictionary_api = https.request(dictionary_url); //why is thre callback in .request optional 
...
function send_results(word, definition, response_code, res) {
    ...
    res.end(results_html); //response back to the client/user res is the response to the user. it was created in request_handler so we keep track of it because we need to respond to the user
}
